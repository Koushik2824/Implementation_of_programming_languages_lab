...
void setOptab()//to setup the initial opcode, this is not changed later
{
    optab["LDA"] = "00";
    optab["LDX"] = "04";
    optab["LDL"] = "08";
...
bool is_number(string s)//to check if string is a number
{
	auto it = s.begin();
...
class Assembler
{
    ...
    vector<string>parsePass2(string line)//for pass2
    {
        vector<string> tokens;
    ...
    vector<string> separate(string op)//if multiple operands are there then , we must separate, like in CLEAR or Indexed addressing
    {
        vector<string> ans;
		int i = 0;
		int l =  op.length();
    ...
    void printmem(int address,bool empty,ofstream &fout)//helper function for intermediate file
    {
        if(empty==false)
        {
    ...
    vector<string> operatorsSeparate(string exp)//for expression evaluation in EQU,WORD,IMMEDIATE AND EXTENDED
    {
		vector<string>res;
        int i=0,j;
    ...
    void negativecheck(int &num,int length)//if negative number is there, this converts it into 2base form.
    {
        if(num<0 && length>2)
        {
            int mask = 0;
    ...
    void insertRegMnemonics(string CSECT)// these rae inserted at the start for all control sections.
    {
        symtab[{CSECT,"A"}]={0,0};
        symtab[{CSECT,"X"}]={0,1};
        symtab[{CSECT,"L"}]={0,2};
    ...
    string removeLeadingTrailingSpaces(string str) {//for correct tokens
...
class ExpressionEvaluator
{
    ...
    {
        this->exp=exp;//sets the expression and control section
        this->value=0;
    ...
    pair<int,int> apply(pair<int,int> o1,pair<int,int> o2,string op)
    {
		...
		if(op == "-") return make_pair(o1.first ^ o2.first, o1.second - o2.second);//if rel-rel then it would become absolute
		if(op == "*") return make_pair(o1.first | o2.first, o1.second * o2.second);
		if(op == "/") return make_pair(o1.first | o2.first, o1.second / o2.second);
		return make_pair(0, 0);
    }
    vector<string> tokenize()//separates all the tokens
	{
		vector<string>res;
        int i=0,j;
        int length=exp.length();
    ...
    bool evaluate()//expressions are evaluated using stack 
    {
        //cout<<"exp:"<<exp<<endl;
        ...
        for(int i=0;i<size;i++)
        {
            ...
            else if(tokens[i]==")")
            {
                while(ops.empty()==false&&ops.top()!="(")
                {
					...
					operands.pop();
                    if(operands.empty())//here if previously some operator is left beacuse of external references that would be taken care
                    {
                    ...
                    {
                        cout<<"Ansolute-Relative is not allowed"<<endl;//given in the textbook
                        return false;
                    }
					ops.pop();
        ...
        if(!operands.empty())
        {
            this->isrelative=operands.top().first;//sets the values for later use
...
bool Assembler::pass1()
{
    ...
    ofstream fout("intermediate.txt");//to store with memory addresses for all instructions
    if(!fin.is_open())
    {
    ...
    string prevCSECT="";//current control section name
        ...
        if(label[0]!='.')//not a comment
        {
            ...
            else
            {
                ...
                if(label!="")
                {
                    ...
                    else  symtab[{prevCSECT,label}]={1,LOCCTR};// as it is relative here 1 is taken
                }
                if(isliteral)
                {
...
bool Assembler::pass2()
{
    ...
    ofstream fout("assemblyListing.txt");//file with object codes
    ...
    string currentText="";
    set<string> extref;
    map<string,vector<string>> modificationRecords;// to store all modification records in each control section
    ...
    while(!fin.eof())
    {
        ...
        if(label[0]!='.')//not a comment
        {
            ...
            else if(opcode=="CSECT")
            {
                ...
                if(currentText!="")
                {
                    stringstream ss;//all text records should be printed
            ...
            {
                if(checkd.find(prevCSECT)!=checkd.end())
                {
                    cout<<"Error duplictae definitions found"<<endl;
                }
                else checkd[prevCSECT]=true;
                fout<<instr<<endl;
                fobject<<"D";
            ...
            {
                if(checkr.find(prevCSECT)!=checkr.end())
                {
                    cout<<"Error duplictae externalrefernces found"<<endl;
                }
                else checkr[prevCSECT]=true;
                fout<<instr<<endl;
                fobject << "R";
                extref.clear();//to store all current external references
            ...
            } else
            {
                ...
                if(optab.find(opcode)!=optab.end())
                {
                        ...
                        else//no multiple operands but operand is there
                        {
                            ...
                            }else
                            {
                                ...
                                if(i&&!n)//immedaie
                                {
                                    ...
                                    else//expression is evaluated below
                                    {
                                ...
                                else if(e==1)//extended instructions expression evaluations
                                {
			        ...
			        else if(e==1 && LOCCTR-prevLOCCTR>2)
			        {
                        ...
                        opcodei |= (i);
                        opcodei |= (n<<1);
			        }//set the operandcode as required
                    fout<<instr;
                    ...
                    fout<<objectcode.str()<<endl;//object code generated and stored in assembly Listing
                ...
                else
                {
                    ...
                    if(opcode=="WORD")//expression evaluated
                    {
                        LOCCTR+=3;
                        if(!is_number(operand))
                        {
                ...
                {
                    if(currentText!="")//here the text records would be printed to the object code
                    {
                        fobject<<"T^";
    ...
    for(auto it:modificationRecords[prevCSECT])//all modification records are printed here
    {
        fobject<<it<<endl;
    }
    fobject<<"E";
    ...
    if(startthere==false)
    {
        cout<<"No start was found, hence output could be wrong"<<endl;
    }
    fin.close();
    fout.close();
    fobject.close();
